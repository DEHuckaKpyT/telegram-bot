...
services:
  ...
  webhook-example-telegram-bot:
    ...
    command: >
      ...
      --telegram-bot.receiving.mode=webhook
      --telegram-bot.receiving.webhook.url-host=https://<YOUR_IP>:8443/api/webhook-example-telegram-bot
      --telegram-bot.receiving.webhook.certificate-path=/cert.pem
      --telegram-bot.receiving.webhook.secret-token-random-generation-print-on-startup=true
    restart: no
    ports:
      - "9001:8080"
```

`telegram-bot.receiving.webhook.url-host` - the URL where Telegram will send webhook updates.

`telegram-bot.receiving.webhook.certificate-path` - the path to your certificate inside the application resources.

`telegram-bot.receiving.webhook.secret-token-random-generation-print-on-startup` - prints the auto-generated value for the `X-Telegram-Bot-Api-Secret-Token` header to the console at startup.
